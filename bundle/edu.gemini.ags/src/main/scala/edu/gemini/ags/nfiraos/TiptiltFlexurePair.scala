  ...
  def pairs(results: List[NfiraosCatalogSearchResults]): List[TiptiltFlexurePair] = {
    ...
    val resultPair = results.foldLeft(pairs) { (p, v) =>
      v.criterion.key match {
        case k if k.starType == NfiraosGuideStarType.tiptilt && k.group.getKey == "CWFS"                                    => (p._1.copy(tt = v.some), p._2)
        ...
        case k if k.starType == NfiraosGuideStarType.flexure && k.group.getKey == "CWFS"                                    => (p._1, p._2.copy(flex = v.some))
      }
    }
