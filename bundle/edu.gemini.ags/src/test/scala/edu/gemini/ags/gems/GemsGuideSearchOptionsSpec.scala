...
class GemsGuideSearchOptionsSpec extends Specification {
  ...
  "GemsGuideSearchOptions" should {
    "provide search options for iris in instrument tip tilt mode" in {
      ...
      criteria.head.key should beEqualTo(GemsCatalogSearchKey(GemsGuideStarType.tiptilt, IrisOdgw.Group.instance))
    ...
    "provide search options for iris in canopus tip tilt mode" in {
      ...
      criteria(1).key should beEqualTo(GemsCatalogSearchKey(GemsGuideStarType.flexure, IrisOdgw.Group.instance))
    ...
    "provide search options for iris in both tip tilt modes" in {
      ...
      criteria(1).key should beEqualTo(GemsCatalogSearchKey(GemsGuideStarType.flexure, IrisOdgw.Group.instance))
      criteria(1).criterion.magConstraint should beEqualTo(MagnitudeConstraints(SingleBand(MagnitudeBand.H), FaintnessConstraint(17.0), Some(SaturationConstraint(8))))
      criteria(2).key should beEqualTo(GemsCatalogSearchKey(GemsGuideStarType.tiptilt, IrisOdgw.Group.instance))
