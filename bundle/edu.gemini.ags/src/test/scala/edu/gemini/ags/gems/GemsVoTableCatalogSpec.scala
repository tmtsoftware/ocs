...
class GemsVoTableCatalogSpec extends Specification {
  ...
  "GemsVoTableCatalog" should {
    "support executing queries" in {
      ...
      val instrument = GemsInstrument.iris
      ...
      results.head.criterion should beEqualTo(GemsCatalogSearchCriterion(GemsCatalogSearchKey(GemsGuideStarType.tiptilt, IrisOdgw.Group.instance), CatalogSearchCriterion("On-detector Guide Window tiptilt", RadiusConstraint.between(Angle.zero, Angle.fromDegrees(0.01666666666665151)), MagnitudeConstraints(SingleBand(MagnitudeBand.H), FaintnessConstraint(14.5), Some(SaturationConstraint(7.3))), Some(Offset(0.0014984027777700248.degrees[OffsetP], 0.0014984027777700248.degrees[OffsetQ])), scala.None)))
    ...
    "preserve the radius constraint for a single item without offsets" in {
      ...
      val key = GemsCatalogSearchKey(GemsGuideStarType.flexure, IrisOdgw.Group.instance)
