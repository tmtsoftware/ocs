...
import edu.gemini.ags.nfiraos.NfiraosUtils4Java;
import edu.gemini.shared.util.immutable.DefaultImList;
import edu.gemini.shared.util.immutable.ImList;
import edu.gemini.ags.nfiraos.NfiraosGuideStars;
import edu.gemini.ags.nfiraos.NfiraosStrehl;
...
class CandidateAsterismsTreeTableModel extends AbstractTreeTableModel {
    ...
    static class Row {
        ...
        private final NfiraosGuideStars _nfiraosGuideStars;
        ...
        // A row corresponding to a NfiraosGuideStars object
        // (top level node, displays the Strehl info and checkbox)
        Row(final NfiraosGuideStars nfiraosGuideStars, final List<Row> children, final Option<Long> when) {
        ...
        Row(final NfiraosGuideStars nfiraosGuideStars, final GuideProbeTargets guideProbeTargets, final MagnitudeBand band, final Option<Long> when) {
            _nfiraosGuideStars = nfiraosGuideStars;
        ...
        NfiraosGuideStars getNfiraosGuideStars() {
        ...
        Boolean getCheck() {
            if (_nfiraosGuideStars != null) {
                final NfiraosStrehl strehl = _nfiraosGuideStars.strehl();
                if (strehl != null) {
        ...
        String getId() {
            ...
            } else if (_nfiraosGuideStars != null) { // top level displays Strehl values
                final NfiraosStrehl strehl = _nfiraosGuideStars.strehl();
                if (strehl != null) {
                    return String.format("%.1f rms", _nfiraosGuideStars.strehl().rms() * 100);
                }
            }
            return null;
        }
        ...
        String getMag() {
            if (_guideProbeTargets != null) {
                if (!_guideProbeTargets.getPrimary().isEmpty()) {
                    ...
                    return NfiraosUtils4Java.probeMagnitudeInUse(guideProbe, _band, _guideProbeTargets.getPrimary().getValue().getMagnitudesJava());
                }
            } else if (_nfiraosGuideStars != null) { // top level displays Strehl values
                final NfiraosStrehl strehl = _nfiraosGuideStars.strehl();
                if (strehl != null) {
                    return String.format("%.1f min", _nfiraosGuideStars.strehl().min() * 100);
                }
            }
            return null;
        }
        ...
        String getRA() {
            if (_guideProbeTargets != null) {
                ...
                final NfiraosStrehl strehl = _nfiraosGuideStars.strehl();
                if (strehl != null) {
                    return String.format("%.1f max", _nfiraosGuideStars.strehl().max() * 100);
                }
            }
            return null;
        }
    ...
    CandidateAsterismsTreeTableModel(final List<NfiraosGuideStars> nfiraosGuideStarsList, final MagnitudeBand band, final Option<Long> when) {
        super(new ArrayList<Row>());
        _columnHeaders = computeColumnHeaders();

        final List<Row> tmp = getRows();

        for(NfiraosGuideStars nfiraosGuideStars : nfiraosGuideStarsList) {
            final List<Row> rowList = new ArrayList<>();
            for (GuideProbeTargets guideProbeTargets : nfiraosGuideStars.guideGroup().getAll()) {
                rowList.add(new Row(nfiraosGuideStars, guideProbeTargets, band, when));
            }
            tmp.add(new Row(nfiraosGuideStars, rowList, when));
        }
    ...
    /**
     * Returns a list of NfiraosGuideStars for the checked asterisms
     */
    List<NfiraosGuideStars> getCheckedAsterisms() {
        final List<NfiraosGuideStars> result = new ArrayList<>();
        if (getRoot() != null) {
            final List<Row> rowList = getRows();
            for (Row row : rowList) {
                if (row.isCheckBoxSelected()) {
                    result.add(row.getNfiraosGuideStars());
                }
            }
        }
        return result;
