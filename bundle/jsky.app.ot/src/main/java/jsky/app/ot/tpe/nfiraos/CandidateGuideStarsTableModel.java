...
class CandidateGuideStarsTableModel extends DefaultTableModel {
    ...
    CandidateGuideStarsTableModel(final NfiraosGuideStarSearchModel model) {
        _model = model;
    ...
    private Vector<Vector<Object>> makeDataVector() {
        _siderealTargets = NfiraosUtils4Java.uniqueTargets(_model.getNfiraosCatalogSearchResults());
    ...
    TableQueryResult getTableQueryResult() {
        ...
        return new SkycatTable(entry, CandidateGuideStarsTableModel.this.getDataVector(), getFields()) {
            ...
            @Override
            public String getId() {
                return "NFIRAOS"; // see entry in ot.skycat.cfg - needed for NfiraosSkyObjectFactory
            }
        };
    }
