...
class PlannedTimeTest extends InstrumentSequenceTestBase[Iris, IrisSeqConfig] {
  override protected def getObsCompSpType = Iris.SP_TYPE
  override protected def getSeqCompSpType = IrisSeqConfig.SP_TYPE
  ...
  private def expected(guiding: Seq[DefaultGuideOptions.Value]): Long = {
    ...
    val (time, _) = ((0.0,offsets.head)/:offsets.tail) { case ((time,prev),cur) =>
      ...
      else (time + Iris.GUIDED_OFFSET_OVERHEAD, cur)
    }
    (time * 1000).round
  }
