...
import static edu.gemini.spModel.gemini.iris.IrisDetectorArray.DETECTOR_GAP_ARCSEC;
import static edu.gemini.spModel.gemini.iris.IrisDetectorArray.DETECTOR_SIZE_ARCSEC;
...
/**
 * Test cases for the IrisOdgw group.
 */
public class IrisOdgwGroupTest extends TestCase {

    private ObsContext baseContext;
    private static final IrisOdgw.Group group = IrisOdgw.Group.instance;
    ...
    // Selection and adding essentially wrap the IrisDetectorArray.getId
    // method with code to map the id to an Odgw or update a TargetEnvironment.
    // Not going to repeat all the IrisDetectorArray.getId tests that have
    ...
    public void testSelect() {
        ...
        for (int i=0; i<4; ++i) {
            ...
            final IrisOdgw expected = IrisOdgw.values()[i];
    ...
    public void testEmptyAdd() {
        ...
        final Option<GuideProbeTargets> gtOpt = env.getPrimaryGuideProbeTargets(IrisOdgw.odgw1);
    ...
    public void testAdd() {
        ...
        for (int i=0; i<4; ++i) {
            final Coordinates coords   = coordsArray[i];
            final SPTarget guideTarget = new SPTarget(coords.getRaDeg(), coords.getDecDeg());
            final IrisOdgw odgw = IrisOdgw.values()[i];
